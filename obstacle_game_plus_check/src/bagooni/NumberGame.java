...
public class NumberGame extends JFrame {
    ...
    public void loadAudio_fail(String pathName) {
		try {
			clip_fail = AudioSystem.getClip();
			File audioFile = new File(pathName);
			AudioInputStream audioStream = AudioSystem.getAudioInputStream(audioFile);
			clip_fail.open(audioStream);
		}
		catch(LineUnavailableException e) {e.printStackTrace();}
		catch(UnsupportedAudioFileException e) {e.printStackTrace();}
		catch(IOException e) {e.printStackTrace();}
    ...
    public void loadAudio_clear(String pathName) {
		try {
			clip_clear = AudioSystem.getClip();
			File audioFile = new File(pathName);
			AudioInputStream audioStream = AudioSystem.getAudioInputStream(audioFile);
			clip_clear.open(audioStream);
		}
		catch(LineUnavailableException e) {e.printStackTrace();}
		catch(UnsupportedAudioFileException e) {e.printStackTrace();}
		catch(IOException e) {e.printStackTrace();}
	...
	public void start() {
		...
		setSize(500, 600);
		setVisible(true);
		loadAudio("images/ant.wav");
		loadAudio_fail("images/fail.wav");
		loadAudio_clear("images/clear.wav");
	}
